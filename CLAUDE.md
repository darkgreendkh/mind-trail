...
- `workbench/` — Workbench wraps everything in `<ReactFlowProvider>`; Toolbar, Canvas, Inspector, and the `useKeyboardShortcuts` hook (Enter=继续主线 / Tab=创建支线 / Delete or Backspace=删除 / Ctrl+Z=撤销 / Ctrl+Shift+Z or Ctrl+Y=重做, all suppressed while typing in an input/textarea — so Ctrl+Z there falls back to the browser's native text undo). RF's own `deleteKeyCode` is disabled (`null`) so Backspace deletes through the store, not RF's local state. Double-clicking a node (`TrailNodeView`) opens an inline title editor that writes straight to the store.
